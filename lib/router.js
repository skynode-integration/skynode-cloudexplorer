...
//      this.unifile.use(new WebDavConnector(options.webdav));
    ...
    this.use('/cloud-explorer/fonts', express.static(Path.resolve(nodeModulesPath('font-awesome'), 'font-awesome/fonts/')));
    this.use('/cloud-explorer/fonts', express.static(Path.resolve(nodeModulesPath('npm-font-open-sans'), 'npm-font-open-sans/fonts/')));
    ...
    this.get('/services', (req, res) => {
      res.send(services.map(service => Object.assign({}, service, this.unifile.getInfos(req.session.unifile, service.name))));
    });
    ...
    this.post('/:connector/logout', (req, res) => {
    ...
    // Register callback url
    this.post('/:connector/login_callback', express.urlencoded({ extended: true }), (req, res) => {
      if (!req.body) Router.handleError(res, new Error('No body parsed supplied'));
